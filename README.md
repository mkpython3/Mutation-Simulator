...
The IT mode serves the same use case as ARGS, though IT is used to generate breakend events at a specified rate. To simulate interchromosomal translocations first two chromosomes will be randomly pared. Secondly the amount of breaks is determined by the specified rate. Each break position is randomly and independently from the break positions of the other chromosome generated within the first and last position of the chromosomes with a minimum distance of one. The sequence after each breakpoint will then be swapped between the partners. As well as ARGS, IT does not provide an option to define a different rate for each chromosome in the reference.
...
To verify Mutation-Simulator is appropriately installed the "test.fa" and "test.rmt" files in the Test directory can be used. If the following test runs succesfully, the installation succeded:
```sh
...
### Create RMT files
RMT files can be written manually in every texteditor. But if data about mutation rate distributions of mutationtypes along a genomic sequence are available in a suitable format like VCF or GTF, an RMT file can be created automatically. 
